...
const Allusers = () => {
  ...
  const [loading, setLoading] = useState(true) // Loading state add kora hoyeche
  ...
  const fetchUsers = async () => {
    try {
      ...
      // Backend jodi error pathay ba data array na hoy, seta handle korbe
      setUsers(Array.isArray(data) ? data : []) 
    } catch (error) {
      console.log("Fetch users error:", error.message)
      setUsers([]) // Error hole empty array set korbe jeno map crash na kore
    } finally {
      setLoading(false)
    }
  }
